    ...
    def _makeTableSpec(self, constraints: bool = False) -> ddl.TableSpec:
        ...
        if datasetColumns is not None:
            ...
            if datasetColumns.rank is not None:
                spec.fields.add(ddl.FieldSpec("rank", dtype=sqlalchemy.Integer, nullable=False))
        return spec
...
class DirectQuery(Query):
    ...
    def sql(self) -> sqlalchemy.sql.FromClause:
        ...
        if datasetColumns is not None:
            ...
            if datasetColumns.rank is not None:
                # Use the query we've built so far as a subquery in the thing
                # we'll actually return, in order to allow that outer query to
                # have a "WHERE rank = 1" constraint instead of a rank return
                # column.
                subquery = simpleQuery.combine()
                # Make a new SimpleQuery for the outer query.  Keep the old one
                # around so we can inspect it a bit.
                outer = SimpleQuery()
                outer.join(subquery)
                outer.columns.extend(subquery.columns[c.name].label(c.name)
                                     for c in simpleQuery.columns[:-1])
                outer.where.append(subquery.columns["rank"] == 1)
                # Replace the original SimpleQuery var with the outer one.
                simpleQuery = outer
